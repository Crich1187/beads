...
func expectIgnoredPending(mock sqlmock.Sqlmock, currentVersion int) {
	expectCursorProbe(mock, "ignored_schema_migrations", true)
	expectScalar(mock, "SELECT COALESCE(MAX(version), 0) FROM ignored_schema_migrations", "version", currentVersion)
	if currentVersion > 0 {
...
func TestRekeyAuxRowIDsSkipsDriftedTable(t *testing.T) {
	...
	expectIgnoredPending(mock, auxRekeyPassInitial.markerVersion-1)
...
func TestRekeyAuxRowIDsResumesDriftedTableOnly(t *testing.T) {
	...
	expectIgnoredPending(mock, auxRekeyPassInitial.markerVersion)
