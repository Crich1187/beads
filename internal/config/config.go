...
func Initialize() error {
	...
	// Lowest to highest user-level precedence: native, documented.
	// Legacy <home>/.beads/config.yaml is deliberately NOT loaded as a global config
	// so that a root workspace config does not leak into unrelated scratch directories (root-8ysaa).
